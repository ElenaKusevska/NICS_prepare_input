...
int periodic_table(std::string& label, double& mass) {

   int k;
	std::vector<std::string> labels{"C", "S"};
	std::vector<double> masses{12.0107, 32.065};

	for (k=0; k<labels.size(); k=k+1) {
      if (label == labels[k]) {
         mass = masses[k];
      }
   }
