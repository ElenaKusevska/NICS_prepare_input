...
int main() {
   ...
   for (i=0; i<nmols; i=i+1) { // for each of the molecules:
      for (j=0; j<nrings_per_molecule[i]; j=j+1) { // for each ring in mol
         ...
         mass.resize(lables_ring.size());
         for (k=0; k<mass.size(); k=k+1) {
            periodic_table(lables_ring[k],mass[k]);
         }
         std::cout << "masses: " << std::endl;
         print_vector_double(mass);
         ...
         inert(2,2) = 0.0;
         for (k=0; k<xring.size(); k=k+1) {
            inert(0,0) = inert(0,0) + mass[k] * (yring[k]*yring[k] + 
                  zring[k]*zring[k]) ;
            inert(0,1) = inert(0,1) - mass[k]*yring[k]*xring[k];
            inert(0,2) = inert(0,2) - mass[k]*xring[k]*zring[k];
            inert(1,1) = inert(1,1) + mass[k] * (xring[k]*xring[k] + 
                  zring[k]*zring[k]) ;
            inert(1,2) = inert(1,2) - mass[k]*yring[k]*zring[k];
            inert(2,2) = inert(2,2) + mass[k] * (xring[k]*xring[k] + 
                  yring[k]*yring[k]) ;
         }
         inert(1,0) = inert(0,1);
         inert(2,0) = inert(0,2);
         ...
         out_ring_xyz_file << rings_atoms[ring_num].size() + 4 << std::endl;
         ...
         out_ring_xyz_file << "N          " << normal1(0) << "         "
                           ...
                           << std::endl;
         for (k=0; k<3; k=k+1) {
            out_ring_xyz_file << "O          " << eigenvecs(k,0).real() << "         "
                              << eigenvecs(k,1).real() << "         " << eigenvecs(k,2).real()
                              << std::endl;
         }
